// ==UserScript==
// @name        ClipSaver for TikTok
// @description Adds a floating download button over TikTok video players that opens a download launcher for the current clip. A companion panel page can trigger downloads and show progress.
// @version     1.0.0
// @license     MIT
...
(function () {
  ...
  var PANEL_TASK = 'xload-6bea7028d046';
  var PANEL_URL = 'https://xload.net/scripts/userscripts/xload-6bea7028d046/panel.html';
  ...
  var TikTok = {
    ...
    getPlayUrl: function (target) {
      var container = (target && target.closest && target.closest('[class*="xgplayer-container"]')) || null;
      if (!container && target) {
        var level = 0;
        var cur = target;
        while (cur && level < 8) {
          if (cur.className && typeof cur.className === 'string' && cur.className.indexOf('xgplayer-container') !== -1) {
            container = cur;
            break;
          }
      ...
      if (container && container.getAttribute('id')) {
        var playId = this.extractLastDashNumber(container.getAttribute('id'));
        if (playId) return 'https://www.tiktok.com/@_/video/' + playId;
      }
      return window.location.href.split('?')[0];
    },
    downloadCurrent: function () {
      Utils.openDownloader(TikTok.getPlayUrl(null));
      try { window.XLoadPanel.log('info', 'download.trigger', { url: window.location.href }); } catch (e) {}
    },
    updateButtonPosition: function () {
      if (!TikTok.hud) return;
      var pathname = window.location.pathname;
      if (/^\/@[^/]+$/.test(pathname)) {
        TikTok.hud.setButtonPosition('bottom-center');
      } else if (/\/@[^/]+\/video\/.+$/.test(pathname)) {
        TikTok.hud.setButtonPosition(document.querySelector("[aria-label='Close']") ? 'right-three-quarter' : 'top-center');
      } else {
        TikTok.hud.setButtonPosition('top-center');
      }
    },
    start: function () {
      if (!/tiktok\.com/.test(window.location.host)) return;
      TikTok.hud = DownloadHud.mount({
        id: 'clipsaver-tiktok-hud',
        targetSelector: '[class*="xgplayer-container"]',
        extraTargetSelector: 'video',
        buttonSize: Panel.config.buttonSize,
        iconSize: Panel.config.iconSize,
        buttonEdgeOffset: Panel.config.edgeOffset,
        getDownloadUrl: function (target) { return TikTok.getPlayUrl(target); }
      });
      Utils.onLocationChange(function () { TikTok.updateButtonPosition(); });
      TikTok.updateButtonPosition();
    }
  };
  ...
  Panel.onApply = function (cfg) {
    if (!TikTok.hud) return;
    if (cfg.edgeOffset != null) TikTok.hud.setButtonPosition(undefined, cfg.edgeOffset);
    if (cfg.enabled) TikTok.hud.enable();
    else TikTok.hud.disable();
  };
