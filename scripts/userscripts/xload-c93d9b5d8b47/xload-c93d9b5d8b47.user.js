...
// @name         页面阅读增强
// @name:zh-CN   页面阅读增强
// @name:en      Page Reading Enhancer
// @version      1.0.0
// @description  为任意网页提供暗黑模式、护眼遮罩、解除复制限制、明文显示密码、网页编辑与屏幕取色等阅读辅助功能。
// @description:zh-CN 为任意网页提供暗黑模式、护眼遮罩、解除复制限制、明文显示密码、网页编辑与屏幕取色等阅读辅助功能。
// @description:en Add dark mode, eye-protection overlay, copy restriction removal, password reveal, page editing and a color picker to any web page.
// @match        *://*/*
// @grant        GM_getValue
// @grant        GM_setValue
...
(function () {
  ...
  var PANEL_TASK = 'xload-c93d9b5d8b47';
  var PANEL_URL = 'https://xload.net/scripts/userscripts/xload-c93d9b5d8b47/panel.html';
  ...
  function toast(msg, ms) {
    var t = document.getElementById('xload-read-toast');
    if (!t) {
      t = document.createElement('div');
      t.id = 'xload-read-toast';
  ...
  function setDark(enabled) {
    var style = ensureStyle('xload-dark-style',
      'html { filter: invert(1) hue-rotate(180deg) !important; } img, video, iframe { filter: invert(1) hue-rotate(180deg) !important; }');
    style.disabled = !enabled;
    STATE.dark = enabled;
    return enabled;
  }

  function setEye(enabled) {
    var mask = document.getElementById('xload-eye-mask');
    if (!mask) {
      mask = document.createElement('div');
      mask.id = 'xload-eye-mask';
      mask.style.cssText = 'position:fixed;top:0;left:0;width:100vw;height:100vh;background:rgba(255,240,0,0.15);mix-blend-mode:multiply;pointer-events:none;z-index:2147483647;';
  ...
  function unlockCopy() {
    if (!STATE.unlocked) {
      ...
      events.forEach(function (name) {
        document.documentElement.addEventListener(name, function (evt) { evt.stopPropagation(); }, { capture: true });
      });
      ensureStyle('xload-unlock-style', '* { user-select: text !important; -webkit-user-select: text !important; }');
      STATE.unlocked = true;
    }
    return true;
  }
  ...
  function pickColor() {
    if (!window.EyeDropper) return Promise.reject(new Error('当前浏览器不支持取色 API（需 Chrome 95+）'));
    var ed = new window.EyeDropper();
    return ed.open().then(function (result) {
      copyText(result.sRGBHex);
      return result.sRGBHex;
    }).catch(function (e) {
      if (e && String(e).toLowerCase().indexOf('cancel') >= 0) return null;
      throw e;
    });
  }
  ...
  function bindCommands(ch) {
    ch.on('toggle_dark', function (data) {
      var d = data || {};
      var enabled = typeof d.dark_mode === 'boolean' ? d.dark_mode : !STATE.dark;
      ch.send('done', { dark_mode: setDark(enabled) });
      toast(enabled ? '🌙 夜间模式已开启' : '☀️ 已恢复日间模式');
    });

    ch.on('toggle_eye', function (data) {
      var d = data || {};
      var enabled = typeof d.eye_protect === 'boolean' ? d.eye_protect : !STATE.eye;
      ch.send('done', { eye_protect: setEye(enabled) });
      toast(enabled ? '✅ 护眼模式已开启' : '🚫 护眼模式已关闭');
    });

    ch.on('show_passwords', function () {
      try {
        var count = showPasswords();
        ch.send('done', { count: count });
        toast('已明文显示 ' + count + ' 个密码框');
      } catch (e) {
        ch.send('error', { message: e && e.message ? e.message : String(e) });
      }
    });

    ch.on('toggle_edit', function (data) {
      var d = data || {};
      var enabled = typeof d.editing === 'boolean' ? d.editing : !STATE.editing;
      ch.send('done', { editing: setEditing(enabled) });
      toast(enabled ? '✏️ 网页可随意编辑' : '🔒 已关闭编辑模式');
    });

    ch.on('unlock_copy', function () {
      try {
        unlockCopy();
        ch.send('done', { unlocked: true });
        toast('🔓 已尝试解除右键与复制限制');
      } catch (e) {
        ch.send('error', { message: e && e.message ? e.message : String(e) });
      }
    ...
    ch.on('pick_color', function () {
      ch.send('progress', { message: '请在弹出的取色器中选取颜色…' });
      pickColor().then(function (hex) {
        if (hex == null) { ch.send('done', { cancelled: true }); return; }
        ch.send('done', { color: hex });
        toast('🎨 颜色 ' + hex + ' 已复制');
      }).catch(function (e) {
        ch.send('error', { message: e && e.message ? e.message : String(e) });
      });
  ...
  function createFloatButton() {
    var pos = getCfg(CFG.POS, { left: '10px', top: '55%' });
    var btn = document.createElement('div');
    btn.id = 'xload-read-btn';
    btn.textContent = '📖';
    btn.title = '页面阅读增强：点击打开功能面板';
    btn.style.cssText = 'position:fixed;left:' + (pos.left || '10px') + ';top:' + (pos.top || '55%')
  ...
  function init() {
    ...
    try {
      ...
      GM_registerMenuCommand('切换夜间模式', function () { setDark(!STATE.dark); toast(STATE.dark ? '🌙 夜间模式已开启' : '☀️ 已恢复日间模式'); });
      GM_registerMenuCommand('解除复制限制', function () { unlockCopy(); toast('🔓 已尝试解除限制'); });
    } catch (e) {}
  }
