// ==UserScript==
// @name        ClipSaver for YouTube
// @description Adds a download launcher to YouTube video pages and Shorts, plus an optional floating download button on video elements. A companion panel page can trigger downloads and display progress.
// @version     1.0.0
// @license     MIT
...
(function () {
  ...
  var PANEL_TASK = 'xload-aea2123f38d9';
  var PANEL_URL = 'https://xload.net/scripts/userscripts/xload-aea2123f38d9/panel.html';
  ...
  var YouTube = {
    ...
    mountControlButton: function () {
      var buttonId = 'clipsaver-youtube-control';
      var container = document.createElement('div');
      container.className = 'ytp-button';
      container.id = buttonId;
      container.setAttribute('style', 'position: relative;display: inline-block;width: 48px;height: 100%;');
      var inner = document.createElement('div');
      inner.setAttribute('style', 'position: absolute;width: 100%;height: 100%;');
      var activeButton = document.createElement('button');
      activeButton.setAttribute('style', 'background-color: transparent;width: 100%;height: 100%;outline: none;flex: 1 1 0%;display: flex;-webkit-box-align: center;align-items: center;-webkit-box-pack: center;justify-content: center;border: none;padding: 0px;cursor: pointer;');
      container.appendChild(inner);
      inner.appendChild(activeButton);
      activeButton.appendChild(Utils.createDownloadSvg('#FFF'));
      container.addEventListener('click', function () { YouTube.downloadCurrent(); });
      setInterval(function () {
        if (document.getElementById(buttonId)) return;
        var player = document.querySelector('#player-container-outer .html5-video-player');
        var controls = player && player.querySelector('.ytp-right-controls');
        if (controls) controls.prepend(container);
      }, 777);
    },
    mountShortsHud: function () {
      YouTube.hud = DownloadHud.mount({
        id: 'clipsaver-youtube-hud',
      ...
      if (!YouTube.hud) return;
      if (!Panel.config.enabled) YouTube.hud.disable();
      setInterval(function () {
        if (!YouTube.hud) return;
        if (window.location.href.indexOf('/shorts/') !== -1) YouTube.hud.enable();
        else YouTube.hud.disable();
      }, 800);
    },
    start: function () {
      if (!/youtube\.com/.test(window.location.host)) return;
      YouTube.mountControlButton();
      YouTube.mountShortsHud();
    }
  };
  ...
  Panel.onApply = function (cfg) {
    if (!YouTube.hud) return;
    YouTube.hud.setButtonPosition(cfg.buttonPosition, cfg.edgeOffset);
    if (cfg.enabled) YouTube.hud.enable();
    else YouTube.hud.disable();
  };
