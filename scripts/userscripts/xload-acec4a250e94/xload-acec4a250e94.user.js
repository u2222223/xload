// ==UserScript==
// @name        ClipSaver for Threads
// @description Adds a download button to Threads posts that contain video, opening a download launcher for the post. A companion panel page can trigger downloads and show progress.
// @version     1.0.0
// @license     MIT
...
(function () {
  ...
  var PANEL_TASK = 'xload-acec4a250e94';
  var PANEL_URL = 'https://xload.net/scripts/userscripts/xload-acec4a250e94/panel.html';
  ...
  /* ------------------------------------------------------------------ */
  var Threads = {
    downloadCurrent: function (url) {
      Utils.openDownloader(url || window.location.href);
      try { window.XLoadPanel.log('info', 'download.trigger', { url: url || window.location.href }); } catch (e) {}
    },
    findPostUrl: function (container) {
      var links = container.querySelectorAll("a[role='link']");
      for (var i = 0; i < links.length; i++) {
        var href = links[i].getAttribute('href');
        if (href && href.indexOf('/post/') !== -1) return 'https://www.threads.com' + href;
      }
      return '';
    },
    inject: function () {
      var posts = document.querySelectorAll("*[data-pressable-container='true']:not([data-clipsaver='true'])");
      Array.prototype.forEach.call(posts, function (element) {
        if (element.querySelector('video') === null) return;
        element.setAttribute('data-clipsaver', 'true');
        var buttons = element.querySelectorAll("*[role='button']");
        if (buttons.length <= 1) return;
        var withNumber = Array.prototype.filter.call(buttons, function (btn) {
          return /\d/.test(btn.textContent || '');
        });
        if (withNumber.length === 0) withNumber = Array.prototype.slice.call(buttons);
        var shareButton = withNumber[withNumber.length - 1].parentElement;
        var downloadButton = shareButton.cloneNode(false);
        downloadButton.appendChild(Utils.createDownloadSvg('#505050', 20, 20));
        downloadButton.style.cursor = 'pointer';
        shareButton.insertAdjacentElement('afterend', downloadButton);
        downloadButton.addEventListener('click', function (e) {
          e.stopPropagation();
          e.preventDefault();
          var url = Threads.findPostUrl(element);
          Threads.downloadCurrent(url || window.location.href);
        });
      });
    },
    start: function () {
      var host = window.location.host;
      if (!/threads\.(com|net)/.test(host)) return;
      setInterval(function () { Threads.inject(); }, Panel.config.pollInterval || 777);
    }
  };
