...
// @allFrames           true
//
// @downloadURL https://update.greasyfork.org/scripts/484611/YouTube%3A%20Audio%20Only.user.js
// @updateURL https://update.greasyfork.org/scripts/484611/YouTube%3A%20Audio%20Only.meta.js
// ==/UserScript==
...
(async function () {
    ...
    const pageInjectionCode = function () {
        ...
        const getYtConfig = () => typeof yt !== 'undefined' ? (yt || 0).config_ : 0;
        ...
        const DIRTY_ALL = 1 | 2 | 4 | 8;

        let skipPlayPause = 0;
        let dirtyMark = DIRTY_ALL;
        ...
        ; (async () => {
            ...
            const rankCandidates = (arr, brr) => {
                const ranked = arr.map(key => [key, brr.get(key) || 0]);
                if (ranked.length > 1) ranked.sort((a, b) => b[1] - a[1]);
                return ranked;
            };

            const detectsAppAccess = (_yt_player, key) => {
                const p = _yt_player[key].prototype;
                const f = p ? p.getPlayerStateObject : null;
                if (!f) return null;
                const accessed = {};
            ...
            const getKeyPlayerDap = (_yt_player, filteredEntries) => {
                ...
                if (arr.length === 0) {
                    ...
                    const ranked = rankCandidates(arr, brr);

                    let match = null;
                    for (const [key] of ranked) {
                        if (detectsAppAccess(_yt_player, key) === true) match = key;
                    }
            ...
            const getKeyStandardApp = (_yt_player, filteredEntries) => {
                ...
                if (arr.length === 0) {
                    ...
                    let match = null;
                    for (const [key] of ranked) {
                        if (detectsAppAccess(_yt_player, key) === false) match = key;
                    }
            ...
            const getKeyInternalApp = (_yt_player, filteredEntries) => {
                ...
                if (arr.length === 0) {
                    ...
                    if (ranked.length > 2) console.log(`[yt-audio-only] (key-extraction) [${w}]`, ranked);
                    return ranked[0][0];
                }
    ...
    const stopPropagationSafely = (evt) => {
        try {
            evt.stopPropagation();
            evt.stopImmediatePropagation();
        } catch (e) { }
