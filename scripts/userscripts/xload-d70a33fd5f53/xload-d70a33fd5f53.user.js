// ==UserScript==
// @name        ClipSaver for XiaoHongShu
// @description Adds a download button to XiaoHongShu note media that opens a download launcher for the current note. A companion panel page can trigger downloads and show progress.
// @version     1.0.0
// @license     MIT
...
(function () {
  ...
  var PANEL_TASK = 'xload-d70a33fd5f53';
  var PANEL_URL = 'https://xload.net/scripts/userscripts/xload-d70a33fd5f53/panel.html';
  ...
  var XiaoHongShu = {
    ...
    ensureStyle: function () {
      if (XiaoHongShu.buttonClass) return XiaoHongShu.buttonClass;
      var className = 'clipsaver-xhs-' + Math.floor(1e5 + Math.random() * 9e5);
      GM_addStyle([
        '.' + className + ' { cursor: pointer; justify-content: center; align-items: center; width: 40px;',
        'height: 40px; z-index: 99999; position: absolute; right: 55px; top: 10px; border-radius: 50%; display: flex; }',
      ...
      XiaoHongShu.buttonClass = className;
      return className;
    },
    inject: function () {
      ...
      if (noteContainer.querySelector('[data-clipsaver="true"]')) return;
      var mediaContainer = noteContainer.querySelector('.media-container');
      if (!mediaContainer) return;
      var className = XiaoHongShu.ensureStyle();
      ...
      downloadElement.appendChild(Utils.createDownloadSvg('#FFF'));
      downloadElement.style.cursor = 'pointer';
      mediaContainer.appendChild(downloadElement);
      downloadElement.addEventListener('click', function () { XiaoHongShu.downloadCurrent(); });
    },
    start: function () {
      if (!/xiaohongshu\.com/.test(window.location.host)) return;
      setInterval(function () { XiaoHongShu.inject(); }, Panel.config.pollInterval || 777);
    }
  };
