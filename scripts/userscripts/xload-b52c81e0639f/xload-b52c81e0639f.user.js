...
// @name:en      Text Reader
// @version      1.0.0
// @description  使用浏览器语音合成朗读选中或输入的文本，可调节语速、音调、音量与语言。
// @description:zh-CN 使用浏览器语音合成朗读选中或输入的文本，可调节语速、音调、音量与语言。
// @description:en Read selected or typed text aloud with the browser speech synthesis API, with adjustable rate, pitch, volume and language.
// @match        *://*/*
// @grant        GM_getValue
// @grant        GM_setValue
...
(function () {
  ...
  var PANEL_TASK = 'xload-b52c81e0639f';
  var PANEL_URL = 'https://xload.net/scripts/userscripts/xload-b52c81e0639f/panel.html';
  ...
  function toast(msg, ms) {
    var t = document.getElementById('xload-tts-toast');
    if (!t) {
      t = document.createElement('div');
      t.id = 'xload-tts-toast';
  ...
  function pickVoice(synth, lang) {
    ...
    var prefix = String(lang || 'zh-CN').split('-')[0];
    var matched = voices.filter(function (v) { return v.lang && v.lang.replace('_', '-').toLowerCase().indexOf(prefix.toLowerCase()) === 0; });
    if (matched.length === 0) return null;
    var preferred = matched.find(function (v) { return /xiaoyi|晓伊/i.test(v.name); });
    return preferred || matched[0];
  }

  function speak(text, opts) {
    var synth = window.speechSynthesis;
    if (!synth) return Promise.reject(new Error('当前浏览器不支持语音合成'));

    return new Promise(function (resolve) {
      function doSpeak() {
        var utterance = new window.SpeechSynthesisUtterance(text);
        var voice = pickVoice(synth, opts.lang);
  ...
  function normalizeNumber(value, def, min, max) {
    var n = Number(value);
  ...
  function bindCommands(ch) {
    ch.on('speak', function (data) {
      var d = data || {};
      var text = (d.text != null && String(d.text).trim()) ? String(d.text).trim() : getSelection();
      if (!text) { ch.send('error', { message: '请输入要朗读的文本或先在页面选中文本' }); return; }
      var opts = {
        lang: d.lang || getCfg(CFG.LANG, 'zh-CN'),
        rate: normalizeNumber(d.rate != null ? d.rate : getCfg(CFG.RATE, 1), 1, 0.1, 10),
        pitch: normalizeNumber(d.pitch != null ? d.pitch : getCfg(CFG.PITCH, 1), 1, 0, 2),
        volume: normalizeNumber(d.volume != null ? d.volume : getCfg(CFG.VOLUME, 1), 1, 0, 1)
      };
      ch.send('progress', { message: '正在朗读…' });
      speak(text, opts).then(function () {
        ch.send('done', { length: text.length });
        toast('朗读完成');
      }).catch(function (e) {
  ...
  function createFloatButton() {
    var pos = getCfg(CFG.POS, { left: '10px', top: '45%' });
    var btn = document.createElement('div');
    btn.id = 'xload-tts-btn';
    btn.textContent = '🗣️';
    btn.title = '文本朗读助手：点击打开功能面板';
    btn.style.cssText = 'position:fixed;left:' + (pos.left || '10px') + ';top:' + (pos.top || '45%')
  ...
  function quickRead() {
    ...
    if (!text) { toast('请先选中要朗读的文本'); return; }
    speak(text, {
      lang: getCfg(CFG.LANG, 'zh-CN'),
      rate: normalizeNumber(getCfg(CFG.RATE, 1), 1, 0.1, 10),
      pitch: normalizeNumber(getCfg(CFG.PITCH, 1), 1, 0, 2),
      volume: normalizeNumber(getCfg(CFG.VOLUME, 1), 1, 0, 1)
    }).then(function () { toast('朗读完成'); }).catch(function (e) {
      toast('朗读失败：' + (e && e.message ? e.message : e));
    });
  }
  ...
  function init() {
    ...
    try {
      ...
      GM_registerMenuCommand('朗读选中文本', quickRead);
    } catch (e) {}
  }
