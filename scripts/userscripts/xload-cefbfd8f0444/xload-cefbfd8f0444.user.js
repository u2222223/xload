// ==UserScript==
// @name        ClipSaver for Facebook
// @description Adds a floating download button over Facebook videos and reels that opens a download launcher for the current video. A companion panel page can trigger downloads and show progress.
// @version     1.0.0
// @license     MIT
...
(function () {
  ...
  var PANEL_TASK = 'xload-cefbfd8f0444';
  var PANEL_URL = 'https://xload.net/scripts/userscripts/xload-cefbfd8f0444/panel.html';
  ...
  var Panel = {
    channel: null,
    onApply: null,
    config: { enabled: true, buttonPosition: 'top-center', buttonSize: 40, iconSize: 25, edgeOffset: 10 },
    apply: function (data) {
  ...
  var Facebook = {
    ...
    getPlayUrl: function () {
      ...
      var match = href.match(/(?:v=|\/videos\/|\/reel\/|\/reels\/)(\d+)/);
      if (match) return 'https://www.facebook.com/reel/' + match[1];
      return href.split('&')[0];
    },
    isVideoPage: function () {
      return /\.facebook\.com\/(reel|reels|videos)\/.*?/.test(window.location.href);
    },
    downloadCurrent: function () {
      Utils.openDownloader(Facebook.getPlayUrl());
      try { window.XLoadPanel.log('info', 'download.trigger', { url: window.location.href }); } catch (e) {}
    },
    start: function () {
      if (!/facebook\.com/.test(window.location.host)) return;
      Facebook.hud = DownloadHud.mount({
        id: 'clipsaver-facebook-hud',
        buttonSize: Panel.config.buttonSize,
        iconSize: Panel.config.iconSize,
        buttonEdgeOffset: Panel.config.edgeOffset,
        getDownloadUrl: function () { return Facebook.getPlayUrl(); }
      });
      setInterval(function () {
        if (!Facebook.hud) return;
        if (Facebook.isVideoPage()) Facebook.hud.enable();
        else Facebook.hud.disable();
      }, 700);
    }
  };
  ...
  Panel.onApply = function (cfg) {
    if (!Facebook.hud) return;
    Facebook.hud.setButtonPosition(cfg.buttonPosition, cfg.edgeOffset);
    if (cfg.enabled) Facebook.hud.enable();
    else Facebook.hud.disable();
  };
