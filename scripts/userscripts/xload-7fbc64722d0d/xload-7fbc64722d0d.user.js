// ==UserScript==
// @name        ClipSaver for TED
// @description Adds a download button to TED talk pages that opens a download launcher for the talk. A companion panel page can trigger downloads and show progress.
// @version     1.0.0
// @license     MIT
...
(function () {
  ...
  var PANEL_TASK = 'xload-7fbc64722d0d';
  var PANEL_URL = 'https://xload.net/scripts/userscripts/xload-7fbc64722d0d/panel.html';
  ...
  var Utils = {
    ...
    onLocationChange: function (callback) {
      ...
      if (!Utils._locationListeners) Utils._locationListeners = [];
      Utils._locationListeners.push(callback);
      if (window.__clipsaver_history_patch__) return;
      window.__clipsaver_history_patch__ = true;
      ...
      var notify = function () {
        ...
        requestAnimationFrame(function () {
          ...
          lastHref = href;
          Utils._locationListeners.forEach(function (cb) {
            try { cb(href); } catch (e) {}
          });
        });
      };
      var patch = function (type) {
        var raw = history[type];
        if (raw.__clipsaver_patched__) return;
        function wrappedState() {
          var ret = raw.apply(this, arguments);
          notify();
          return ret;
        }
        wrappedState.__clipsaver_patched__ = true;
  ...
  var Ted = {
    ...
    downloadCurrent: function () {
      Utils.openDownloader(window.location.href);
      try { window.XLoadPanel.log('info', 'download.trigger', { url: window.location.href }); } catch (e) {}
    },
    ensureStyle: function () {
      if (Ted.buttonClass) return Ted.buttonClass;
      var className = 'clipsaver-ted-' + Math.floor(1e5 + Math.random() * 9e5);
      GM_addStyle([
        '.' + className + ' { cursor: pointer; justify-content: center; align-items: center; width: 40px; height: 40px;',
        'z-index: 999999; position: relative; border-radius: 50%; display: flex; }',
        '.' + className + '::before { content: ""; position: absolute; inset: 0; border-radius: inherit; padding: 2px;',
        'background: linear-gradient(90deg,red,orange,yellow,green,cyan,blue,violet,red); background-size: 200% 200%;',
        'opacity: 0; transition: opacity .25s; -webkit-mask: linear-gradient(#000 0 0) content-box, linear-gradient(#000 0 0);',
        '-webkit-mask-composite: xor; mask-composite: exclude; }',
        '.' + className + ':hover::before { opacity: 1; animation: clipsaver-rainbow 3s linear infinite; }',
        '@keyframes clipsaver-rainbow { 0% { background-position: 0% 50%; } 100% { background-position: 200% 50%; } }'
      ].join('\n'));
      Ted.buttonClass = className;
      return className;
    },
    isTalkPage: function () {
      return /www\.ted\.com\/talks\//.test(window.location.href);
    },
    createButton: function () {
      var className = Ted.ensureStyle();
      var downloadElement = document.createElement('div');
      downloadElement.setAttribute('data-clipsaver', 'true');
      downloadElement.classList.add(className);
      downloadElement.style.cursor = 'pointer';
      downloadElement.appendChild(Utils.createDownloadSvg('#000'));
      downloadElement.addEventListener('click', function () { Ted.downloadCurrent(); });
      return downloadElement;
    },
    injectIfNeeded: function () {
      if (!Ted.isTalkPage() || !Panel.config.enabled) return;
      var className = Ted.buttonClass;
      if (className && document.querySelector('.' + className)) return;
      var title = document.querySelector('#talk-title');
      var actionContainer = title && title.parentNode ? title.parentNode.querySelector('.items-center') : null;
      if (!actionContainer) return;
      actionContainer.appendChild(Ted.createButton());
    },
    start: function () {
      if (!/ted\.com/.test(window.location.host)) return;
      var rafId = null;
      ...
      Utils.onLocationChange(scheduleInject);
      scheduleInject();
    }
  };
