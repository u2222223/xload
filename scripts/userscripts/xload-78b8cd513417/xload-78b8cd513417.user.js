// ==UserScript==
// @name        ClipSaver for Pinterest & Snapchat
// @description Adds a floating download button over Pinterest and Snapchat video content that opens a download launcher for the current item. A companion panel page can trigger downloads and show progress.
// @version     1.0.0
// @license     MIT
...
(function () {
  ...
  var PANEL_TASK = 'xload-78b8cd513417';
  var PANEL_URL = 'https://xload.net/scripts/userscripts/xload-78b8cd513417/panel.html';
  ...
  var Pinterest = {
    ...
    getPlayUrl: function (target) {
      var anchor = target && target.closest ? target.closest('a[href*="/pin/"]') : null;
      return anchor ? anchor.href : window.location.href;
    },
    start: function () {
      if (!/pinterest\.com/.test(window.location.host)) return;
      Pinterest.hud = DownloadHud.mount({
        id: 'clipsaver-pinterest-hud',
        buttonSize: Panel.config.buttonSize,
        iconSize: Panel.config.iconSize,
        buttonEdgeOffset: Panel.config.edgeOffset,
        getDownloadUrl: function (target) { return Pinterest.getPlayUrl(target); }
      });
      if (Pinterest.hud && !Panel.config.enabled) Pinterest.hud.disable();
    }
  };
  ...
  var Snapchat = {
    ...
    isSnapchatUrl: function (url) {
      return Snapchat.PATTERNS.some(function (re) { return re.test(url); });
    },
    start: function () {
      if (!/snapchat\.com/.test(window.location.host)) return;
      Snapchat.hud = DownloadHud.mount({
        id: 'clipsaver-snapchat-hud',
        buttonSize: Panel.config.buttonSize,
        iconSize: Panel.config.iconSize,
        buttonEdgeOffset: Panel.config.edgeOffset,
        getDownloadUrl: function () { return window.location.href; }
      });
      var updateHud = function () {
        if (!Snapchat.hud) return;
        if (Snapchat.isSnapchatUrl(window.location.href)) Snapchat.hud.enable();
        else Snapchat.hud.disable();
      };
      Utils.onLocationChange(updateHud);
      updateHud();
    }
  };
  ...
  var ACTIONS = {
    download_current: function (data) {
      Panel.apply(data);
      if (/pinterest\.com/.test(window.location.host)) Utils.openDownloader(Pinterest.getPlayUrl(null));
      else Utils.openDownloader(window.location.href);
    },
  ...
  Panel.onApply = function (cfg) {
    [Pinterest.hud, Snapchat.hud].forEach(function (hud) {
      if (!hud) return;
      hud.setButtonPosition(cfg.buttonPosition, cfg.edgeOffset);
      if (cfg.enabled) hud.enable();
      else hud.disable();
    });
  };
