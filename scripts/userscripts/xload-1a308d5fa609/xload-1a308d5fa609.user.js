...
// @version      1.0.0
// @description  将选中文本或输入的网址、文字一键生成二维码，支持尺寸与容错级别设置。
// @description:zh-CN 将选中文本或输入的网址、文字一键生成二维码，支持尺寸与容错级别设置。
// @description:en Turn selected text or typed content into a QR code with configurable size and error-correction level.
// @match        *://*/*
// @grant        GM_getValue
// @grant        GM_setValue
...
(function () {
  ...
  var PANEL_TASK = 'xload-1a308d5fa609';
  var PANEL_URL = 'https://xload.net/scripts/userscripts/xload-1a308d5fa609/panel.html';
  ...
  function toast(msg, ms) {
    var t = document.getElementById('xload-qr-toast');
    if (!t) {
  ...
  function showQrModal(text, dataUrl) {
    var old = document.getElementById('xload-qr-overlay');
    if (old && old.parentNode) old.parentNode.removeChild(old);

    var overlay = document.createElement('div');
    overlay.id = 'xload-qr-overlay';
    ...
    box.style.cssText = 'background:#fff;border-radius:12px;padding:20px;max-width:90vw;text-align:center;box-shadow:0 20px 60px rgba(0,0,0,.3);';

    var img = document.createElement('img');
    img.src = dataUrl;
    img.style.cssText = 'width:256px;height:256px;max-width:70vw;display:block;';
    box.appendChild(img);

    var tip = document.createElement('div');
    tip.textContent = text.length > 100 ? text.slice(0, 100) + '…' : text;
    tip.style.cssText = 'font-size:12px;color:#999;margin-top:10px;word-break:break-all;max-width:256px;line-height:1.4;';
    box.appendChild(tip);
  ...
  function generateQr(text, size, level) {
    return new Promise(function (resolve, reject) {
  ...
  function bindCommands(ch) {
    ch.on('generate', function (data) {
      var d = data || {};
      var text = (d.content != null && String(d.content).trim()) ? String(d.content).trim() : (getSelection() || location.href);
      if (!text) { ch.send('error', { message: '没有可生成二维码的内容' }); return; }
      var size = Math.max(64, Math.min(1024, Number(d.size) || getCfg(CFG.SIZE, 256)));
      var level = d.error_level || getCfg(CFG.LEVEL, 'M');
      ch.send('progress', { message: '正在生成二维码…' });
      generateQr(text, size, level).then(function (dataUrl) {
        ch.send('done', { text: text, dataUrl: dataUrl, size: size, level: level });
        showQrModal(text, dataUrl);
        toast('二维码已生成');
      }).catch(function (e) {
  ...
  function createFloatButton() {
    var pos = getCfg(CFG.POS, { left: '10px', top: '35%' });
    var btn = document.createElement('div');
    btn.id = 'xload-qr-btn';
    btn.textContent = '📱';
    btn.title = '二维码生成器：点击打开功能面板';
    btn.style.cssText = 'position:fixed;left:' + (pos.left || '10px') + ';top:' + (pos.top || '35%')
  ...
  function quickGenerate() {
    var text = getSelection() || location.href;
    if (!text) { toast('请先选中要生成二维码的文本'); return; }
    toast('正在生成二维码…');
    generateQr(text, getCfg(CFG.SIZE, 256), getCfg(CFG.LEVEL, 'M')).then(function (dataUrl) {
      showQrModal(text, dataUrl);
    }).catch(function (e) {
  ...
  function init() {
    ...
    try {
      ...
      GM_registerMenuCommand('为选中文本生成二维码', quickGenerate);
    } catch (e) {}
  }
