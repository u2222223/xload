...
(function () {
  ...
  // ---- 面板入口（独立页弹窗：window.open + moveTo 居中，便于后续接 AdSense）----
  // 说明：Google AdSense 政策禁止在 iframe 内嵌广告，也禁止把广告注入第三方页面（抖音）。
  //       广告必须加载在我们自有域名 xload.net 的独立页面（panel.html）上。
  //       因此面板保持独立页，用 window.open 打开；用 popup=yes + moveTo/resizeTo 兜底居中。
  function openPanel() {
    if (!isSafeUrl(PANEL_URL)) { log('panel.open.blocked', { url: PANEL_URL }); return false; }
    restoreLogs();
    log('panel.open', { url: PANEL_URL });
    try {
      var W = Math.min(860, Math.max(420, (window.screen.availWidth || 1280) - 120));
      var H = Math.min(720, Math.max(480, (window.screen.availHeight || 800) - 140));
